...
\pl{need to scope this to hard NLP tasks, which is the overarching motivation}
...
In \pl{the field of} natural language processing (NLP) field,
  there is a renewed discussion of how the performance of current NLP systems drops when applied to the real world~\citet{plank16nonstandard} \pl{citep}:
...
\begin{figure}
  ...
  First, we must \pl{drop modal} identify a \textit{source of inputs} (e.g.\ documents) or a distribution over the same that are provided to the system.
...
The evaluation designer then reifies \pl{not sure this is the correct word usage} the various goals of evaluation by specifying a \textit{distribution} over system output and input examples.
...
Finally, the evaluation framework we presented does not yet cover natural language tasks that need human interaction, e.g.\ chatbots \pl{use the word 'dialogue system' instead}. \pl{cite dialogue evaluation paper}
...
We exploit \pl{don't switch from past to present tense} advances in crowdsourcing to collect human feedback to evaluate systems as necessary and develop new statistical estimators to make efficient use of the feedback.
