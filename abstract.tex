...
Existing fully-automatic methods for evaluating these systems rely on an incomplete \pl{italicize important words like 'incomplete'} set of annotated references which lead to systematic biases against certain system improvements: in other words, genuinely good ideas are systematically discarded simply because of limitations in our evaluation methodology.
As a result, human evaluation, which can be prohibitively expensive, has remained the de-facto \pl{not quite - it's the gold standard, but not everyone does this (as you point out later, people use ROUGE} mode of evaluation for these tasks.
...
\pl{need better transition - we're talking about using humans in evaluation; this is moving to something different}
...
Our key statistical \pl{what's statistical about this idea?} idea here is to cast the problem as a stochastic game based on Bayesian decision theory, which allows us to balance latency, cost, and accuracy objectives in a principled way.
