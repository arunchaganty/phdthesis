...
def do_command(args):
    ...
    xy = [[int(datum['prompts'][prompt]['gold'] > 0), datum['prompts'][prompt][metric]]  for datum in data if datum['system'] == system]
    xy = np.array(xy)
    ...
    plt.violinplot([y[x==0], y[x==1]], [0, 1])
    plt.scatter(x, y, alpha=0.3, marker='.')

    coeffs = np.polyfit(x,y, 1)
    plt.plot([0,1], [coeffs[1], coeffs[0] + coeffs[1]], ":")
#
#
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--input', type=str, default="msmarco/msmarco_data.json", help="")
    parser.add_argument('-o', '--output', type=str, default="msmarco_bias.pdf", help="An example trajectory for a task")
