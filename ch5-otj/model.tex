...
\begin{figure}
  ...
  \caption[Incorporating information from responses.]{\label{fig:behavior}
  ...
  See the paragraph on behavior in \refsec{model} for more information.
  }
\end{figure}
...
\begin{figure}
  \centering
      ...
      \caption[Game tree.]{\label{fig:tree}
      Example behavior while running structure prediction on the tweet ``Soup on George str.''
...
\paragraph{Game tree.}
Let us now formalize the game tree in terms of its states, actions, transitions and rewards; see \reffig{tree} for an example. 
...
This is intractable to compute exactly, so we propose an approximate algorithm in \refsec{game-playing}.

%\refeqn{utility} is related to \refeqn{value} by considering the utility at the time of the last request, $\tmax = \max\{t_1, \ldots, t_k\}$: $U(\bq, \bs, \br) = \E_{\bt}[U_{\tmax}(\bq, \bs, \br, \bt)]$.
...
% Example time!
%Let us look at the example in \reffig{game-tree} more intuitively:
...
Given this full model, we can compute $p(r' \mid \bx, \br, q)$ simply by marginalizing out $\by$ and $\bt$ from \refeqn{dynamics}.
When conditioning on $\br$, we ignore responses that have not yet been received (i.e.\ when $r_j = \emptyset$ for some $j$).
%Given this full model, we can compute $p(\br \mid \bx, \bq)$ simply by marginalizing out $\by$ and $\bt$ from \refeqn{dynamics}.
...
\reffig{behavior} shows how the marginals over the labels change as the crowd provides responses for our running example, i.e.\ named entity recognition for the sentence ``Soup on George str.''.
...
%For example,
%in \reffig{game-tree}, getting the crowd to label on the token ``George'' provides information not only to its label, but also to the adjacent token ``str.''. Hence, the system prioritizes asking ``George'' over asking for ``str.''. %\todo{Update figure, and have actual numbers here.}

%in the model in \reffig{game-tree}, the model, a linear-chain CRF, propagates information between adjacent labels. 
...
%\end{align}
%
%Let us revisit the example in \reffig{game-tree} by looking at the game tree on the right where the system turns in an answer after a single query.
