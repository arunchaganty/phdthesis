...
def do_command(args):
    ...
    plt.savefig(pdf_build(args.output, 1))

    ax.errorbar(np.arange(1,len(systems)+1), X, yerr=[X - Y, X-X], fmt='o', color='k', linestyle='-', capsize=0, alpha=0.7)
    ax.plot(np.arange(1,len(systems)+1), Y, color='g', marker='^', linestyle='', alpha=0.9, label="Unpooled score")
    fig.set_tight_layout(True)
    legend = ax.legend(fontsize=16)
    legend.set_alpha(0.5)
    plt.savefig(pdf_build(args.output, 2))
    ...
    legend = ax.legend(fontsize=16)
    legend.set_alpha(0.5)
    plt.savefig(pdf_build(args.output, 3))
