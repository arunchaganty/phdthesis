...
def do_command(args):
    ...
    for i, series in enumerate(["p", "r", "f1"]):
        ...
        Xs = 1 + np.arange(len(data["simple"][i]))
        ...
        plt.savefig(pdf_build(outputs[series],0))

        for kix, k in enumerate(["pool", "simple", "joint"]):
            ...
            ax.legend()
            fig.set_tight_layout(True)
            plt.savefig(pdf_build(outputs[series],kix+1))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-op', '--output-precision', type=str, default='simulation/simulation-p.png', help="Name of file to output to")
    parser.add_argument('-or', '--output-recall', type=str, default='simulation/simulation-r.png', help="Name of file to output to")
    parser.add_argument('-of', '--output-f1', type=str, default='simulation/simulation-f1.png', help="Name of file to output to")
