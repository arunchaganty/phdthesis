\section{Background}
\label{sec:kbpo:setup}
% (1 page)
\begin{figure}[t]
  \centering
  \includegraphics[width=0.5\columnwidth]{figures/pooling}
  \caption[Pooled evaluation]{\label{fig:kbpo:pooling}
 ...
 % Arrows indicate which systems extract which relation instances.
  However, when a new system (C) is evaluated on this dataset, some of its predictions ($i_6$) are missing and cannot be fairly evaluated.
...
In the example shown in \reffig{kbpo:example},
...
In the example in \reffig{kbpo:pooling}, systems A and B were used in constructing the pooling dataset, and there are 3 distinct relations in the dataset, between $s_1$ and $o_1$, $o_2$, $o_3$.

A system is evaluated on the precision of its predicted relation instances for the evaluation entities $E$ and on the recall of the corresponding predicted \textit{relations} (not instances) for the same entities (see \reffig{kbpo:pooling} for a worked example).
...
%These tuples cannot be assessed without labeling the predictions and are considered to be wrong by default.
...
%Let's start with an example to understand how entities and relations are defined: \reffig{example} discusses the late actress Carrie Fisher.
%
...
%%Let us now see an example of how to score the output of two systems $A$ and $B$, summarized in \reffig{evaluation-table}.
%
...
%
%Another common heuristic used by teams developing systems for TAC-KBP is the \anydoc{} heuristic which treats any relation whose object string (e.g.\ ``Debbie Reynolds'' in \reffig{example}) exactly matches that of a correct relation instance in the evaluation set.
