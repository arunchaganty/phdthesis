\chapter[Supplementary material for Chapter 3]{\label{chap:kbpo-appendix} Supplementary material for \refchap{kbpo}} 

\section{Theoretical proofs for the sampling procedures}
\label{sec:sampling}

Let's refresh notation from \refsec{kbpo:method}.
...
In this section, we'll provide proofs that show that the joint estimators proposed in \refsec{kbpo:method} are indeed unbiased, and we will characterize their variance.

\subsection{Estimating precision}

In \refsec{kbpo:method}, we proposed the following estimator for $\pi_i$:
\begin{align*}
...
In \refsec{kbpo:method}, we used the fact that the recall of system $i$, $\recall_i$, can be expressed as the recall of $i$ within the pool, $\nu_i$ and the recall of the pool itself $\theta$: $\recall_i = \theta \nu_i$:  
\begin{align*}
...
In \refsec{kbpo:joint}, we outlined a method to pick the optimal number of samples to draw and evaluate for a new system:
