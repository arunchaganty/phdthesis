...
\begin{figure}[t]
  ...
  \begin{subfigure}{\columnwidth}
      \includegraphics[width=\columnwidth]{figures/pooling_bias_f1/pooling_bias/pooling_bias}
    \centering
...
% %Let's assume that our evaluation set contains only the relation instance shown in \reffig{example}, which identifies Debbie Reynolds as a parent of Carrie Fisher.
...
% 
% \paragraph{Results.}
% \reffig{pooling-bias} summarizes our results on pooling bias for the macro \fone{} metric.
...
% %\reffig{pooling-bias} shows the pooling bias experienced by teams and systems in the 2015 evaluation;
% %  the median bias against an unpooled team is 13.55\% \fone{} points and for an unpooled system is 4.55\% \fone{} points,
% %  making it impossible to evaluate against this metric in development (also significantly biasing new entrants to the task).
% %
% 
...
% %With this heuristic (\reffig{pooling-bias-anydoc}), the pooling bias decreases considerably, with 
...
% % We use a bootstrap sampling \pl{what exactly is being resampled?} method to estimate the variance of $\fone$ and other metrics over query sets \tableref{metric-variance} and measure the 95\% confidence intervals for system scores \reffig{f1}.
...
% % 
% % \reffig{sf1} plots the performance of systems under the standardized macro \fone metric.
